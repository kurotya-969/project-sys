...
export class GameState {
    constructor(config = {}) {
        ...
        this.config = Object.assign({
            goal_money: 75000,            // 目標金額（バランス調整: 100000→75000）
            affection_threshold: 70,      // 好感度閾値
            max_days: 20,                // 最大日数（30日→20日に短縮）
        ...
        // 表示済みイベント追跡（順次表示用）
        this.playEventIndex = 0;  // 次に表示するplayイベントのインデックス
        this.workEventIndex = 0;  // 次に表示するworkイベントのインデックス

        // ターニングポイントイベント達成フラグ
        this.turningPointsShown = {
            affection_60: false,
            money_50000: false,
            affection_75: false,
            money_75000: false
    ...
    applyAction(actionType, eventData = {}) {
        // イベントデータがある場合はイベントの効果を使用
        const hasEventEffects = eventData.affection_delta !== undefined || eventData.money_delta !== undefined;
    ...
    getState() {
        return {
            ...
            playEventIndex: this.playEventIndex,
            workEventIndex: this.workEventIndex,
            turningPointsShown: { ...this.turningPointsShown },
            config: { ...this.config }
        };
    }
    ...
    setState(state) {
        ...
        this.playEventIndex = state.playEventIndex || 0;
        this.workEventIndex = state.workEventIndex || 0;

        if (state.turningPointsShown) {
            this.turningPointsShown = { ...state.turningPointsShown };
        }
    ...
    checkEndingCondition() {
        ...
        // 最大日数経過時のエンディング判定（要件4.1）
        if (this.day >= this.config.max_days) {
            // 理想の共存エンド：好感度70以上かつ所持金75000円以上（要件4.2）
            if (this.affection >= this.config.affection_threshold &&
                this.money >= this.config.goal_money) {
                return 'perfect_end';
            }

            // 夢を叶えるエンド：所持金75000円以上（要件4.3）
