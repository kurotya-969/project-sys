...
class VisualManager {
    ...
    /**
     * カスタムキャラクター画像を適用する（Canvas編集で作成した画像用）
     * @param {string} imageDataUrl - Data URL形式の画像
     */
    applyCustomCharacterImage(imageDataUrl) {
        if (!this.characterImage) {
            console.error('Character image element not found');
            return;
        }

        this.characterImage.src = imageDataUrl;
        this.characterImage.alt = 'カスタムしす';
        console.log('Custom character image applied');
    }
